...
let AcademyTeamsService = class AcademyTeamsService {
     ...
     */
    handleTeamNameUniqueConstraint(error) {
        if (error instanceof
            Prisma.PrismaClientKnownRequestError &&
            error.code === 'P2002') {
            throw new ConflictException('An active team with this name already exists in this academy');
        }
        throw error;
    }
    // =========================================================
    ...
    async createTeam(userId, academyId, dto) {
        ...
        const normalizedName = this.normalizeTeamName(dto.name);
        /**
         * Prevent duplicate ACTIVE teams
         * with the same name in the academy.
         */
        const activeTeam = await this.prisma.academyTeam.findFirst({
            where: {
                academyId,
                deletedAt: null,
                name: {
         ...
         */
        const deletedTeam = await this.prisma.academyTeam.findFirst({
            where: {
                academyId,
                deletedAt: {
                    not: null,
        ...
        if (deletedTeam) {
            try {
                const reactivated = await this.prisma.academyTeam.update({
                    where: {
                        id: deletedTeam.id,
                    },
                    data: {
                ...
                this.handleTeamNameUniqueConstraint(error);
            }
        }
        /**
    ...
    async updateTeam(userId, academyId, teamId, dto) {
        ...
        if (dto.name !== undefined) {
            ...
            const duplicateTeam = await this.prisma.academyTeam.findFirst({
                where: {
                    academyId,
                    deletedAt: null,
                    id: {
    ...
    async addPlayer(userId, academyId, teamId, dto) {
        ...
        if (existing) {
            ...
            return {
                success: true,
                message: 'Player team membership reactivated successfully',
                data: reactivated,
            };
        }
        /**
    ...
    async addCoach(userId, academyId, teamId, dto) {
        ...
        if (existing) {
             ...
             */
            const reactivated = await this.prisma.academyTeamCoach.update({
                where: {
                    id: existing.id,
                },
                data: {
            ...
            return {
                success: true,
                message: 'Coach team membership reactivated successfully',
                data: reactivated,
            };
        }
        /**
    ...
    async removeCoach(userId, academyId, teamId, coachId) {
        ...
        if (!membership ||
            membership.leftAt) {
            throw new NotFoundException('Coach is not an active member of this team');
        }
