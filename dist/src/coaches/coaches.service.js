...
let CoachesService = class CoachesService {
    ...
    async verifyCoachRole(userId) {
        ...
        const isCoach = user.roles.some((userRole) => userRole.role.name === 'COACH');
        if (!isCoach) {
            throw new ForbiddenException('Only coach users can manage a coach profile');
        }
        return user;
    }
    /**
     ...
     */
    validateCoachAchievementType(achievementType) {
        if (!COACH_ACHIEVEMENT_TYPES.includes(achievementType)) {
            throw new BadRequestException('This achievement type is not allowed for coach profiles');
        }
    }
    // =========================================================
    // COACH PROFILE
    ...
    async createProfile(userId, dto) {
        ...
        if (existingProfile &&
            existingProfile.deletedAt !== null) {
            const restoredProfile = await this.prisma.coachProfile.update({
                where: {
                ...
                data: {
                    ...
                    state: dto.state,
                    city: dto.city,
                    currentAcademyClub: dto.currentAcademyClub,
                    coachingRole: dto.coachingRole,
                    coachingLicense: dto.coachingLicense,
                    coachingCertification: dto.coachingCertification,
    ...
    async getMyProfile(userId) {
        const coach = await this.verifyCoachOwner(userId);
        const profile = await this.prisma.coachProfile.findUnique({
            where: {
                id: coach.id,
            },
    ...
    async updateProfile(userId, dto) {
        const profile = await this.verifyCoachOwner(userId);
    ...
    async getProfileById(coachId) {
        await this.findActiveCoach(coachId);
    ...
    async createAchievement(userId, dto) {
        const coach = await this.verifyCoachOwner(userId);
        this.validateCoachAchievementType(dto.achievementType);
        const achievement = await this.prisma.achievement.create({
            data: {
                 ...
                 * or coachId.
                 */
                ownerType: AchievementOwnerType.COACH,
                coachId: coach.id,
                title: dto.title,
        ...
        return {
            success: true,
            message: 'Coach achievement created successfully',
            data: achievement,
        };
    }
    /**
    ...
    async getMyAchievements(userId) {
        ...
        return {
            success: true,
            message: 'Coach achievements retrieved successfully',
            data: achievements,
        };
    }
    /**
    ...
    async updateAchievement(userId, achievementId, dto) {
        ...
        if (!achievement) {
            throw new NotFoundException('Coach achievement not found');
        }
        ...
        return {
            success: true,
            message: 'Coach achievement updated successfully',
            data: updated,
        };
    }
    /**
    ...
    async deleteAchievement(userId, achievementId) {
        ...
        return {
            success: true,
            message: 'Coach achievement deleted successfully',
            data: null,
        };
    }
    // =========================================================
     ...
     */
    async followCoach(userId, coachId) {
        const coach = await this.findActiveCoach(coachId);
        if (coach.userId === userId) {
            throw new ForbiddenException('You cannot follow your own coach profile');
        }
        const existingFollow = await this.prisma.coachFollower.findUnique({
            where: {
                coachId_userId: {
                    coachId,
                    userId,
        ...
        if (existingFollow) {
            return {
                success: true,
                message: 'Already following coach',
                data: {
                    following: true,
                },
            };
        ...
        return {
            success: true,
            message: 'Coach followed successfully',
            data: {
                following: true,
            },
        };
     ...
     */
    async unfollowCoach(userId, coachId) {
        await this.findActiveCoach(coachId);
        const existingFollow = await this.prisma.coachFollower.findUnique({
            where: {
                coachId_userId: {
        ...
        if (!existingFollow) {
            return {
                success: true,
                message: 'Coach is not being followed',
                data: {
                    following: false,
                },
            };
        ...
        return {
            success: true,
            message: 'Coach unfollowed successfully',
            data: {
                following: false,
            },
        };
     ...
     */
    async isFollowingCoach(userId, coachId) {
        await this.findActiveCoach(coachId);
        const follow = await this.prisma.coachFollower.findUnique({
            where: {
                coachId_userId: {
                    coachId,
                    userId,
        ...
        return {
            success: true,
            message: 'Coach follow status retrieved successfully',
            data: {
                following: !!follow,
            },
        };
    ...
    async getCoachFollowerCount(coachId) {
        await this.findActiveCoach(coachId);
        const followers = await this.prisma.coachFollower.count({
            where: {
                coachId,
            },
        });
        return {
            success: true,
            message: 'Coach follower count retrieved successfully',
            data: {
                followers,
            },
        };
     ...
     */
    async likeCoach(userId, coachId) {
        const coach = await this.findActiveCoach(coachId);
        if (coach.userId === userId) {
            throw new ForbiddenException('You cannot like your own coach profile');
        }
        const existingLike = await this.prisma.coachLike.findUnique({
            where: {
                coachId_userId: {
                    coachId,
                    userId,
     ...
     */
    async unlikeCoach(userId, coachId) {
        await this.findActiveCoach(coachId);
        const existingLike = await this.prisma.coachLike.findUnique({
            where: {
                coachId_userId: {
        ...
        return {
            success: true,
            message: 'Coach unliked successfully',
            data: {
                liked: false,
            },
        };
     ...
     */
    async isCoachLiked(userId, coachId) {
        await this.findActiveCoach(coachId);
        const like = await this.prisma.coachLike.findUnique({
            where: {
                coachId_userId: {
                    coachId,
                    userId,
        ...
        return {
            success: true,
            message: 'Coach like status retrieved successfully',
            data: {
                liked: !!like,
            },
        };
    ...
    async getCoachLikesCount(coachId) {
        await this.findActiveCoach(coachId);
        const count = await this.prisma.coachLike.count({
            where: {
                coachId,
            },
        });
        return {
            success: true,
            message: 'Coach likes count retrieved successfully',
            data: {
                count,
            },
        };
