...
async function main() {
    ...
    for (const roleName of Object.values(RoleName)) {
        const desiredPermissionNames = rolePermissions[roleName];
        ...
        if (desiredPermissions.length !==
            ...
            const foundNames = new Set(desiredPermissions.map((permission) => permission.name));
            const missingNames = desiredPermissionNames.filter((name) => !foundNames.has(name));
            throw new Error(`Missing permissions for ${roleName}: ${missingNames.join(', ')}`);
        }
        ...
        console.log(`${roleName}: ${desiredPermissions.length} permission(s) synchronized.`);
    }
    console.log('RBAC seed completed successfully.');
    console.log('No users or passwords were modified.');
}
main()
    .catch((error) => {
